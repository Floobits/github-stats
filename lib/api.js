...
function stargazers_for_repo (org, repo, cb) {
  const url = `https://api.github.com/repos/${org}/${repo}/stargazers`;
  recursive_request(url, ["login"], function (err, res) {
    log.log(res.length ? "found " + res.length : "no", "stargazers for", org, repo);
    return cb(err, res);
  });
}

function starings_for_user (user, cb) {
  ...
  recursive_request(url, fields, reqOptions, cb);
}

function repos_for_orgs (org, cb) {
  const url = `https://api.github.com/orgs/${org}/repos`;
  recursive_request(url, [{repo: 'name'}, {fork: 'fork'}], cb);
}
