...
function stargazers_for_repo (stargazers, org, repo, cb) {
  const url = `https://api.github.com/repos/${org}/${repo}/stargazers`;
  recursive_request(url, stargazers, {org: org, repo: repo}, ["login"], function (err, res) {
    log.log(res.length ? "found " + res.length : "no", "stargazers for", org, repo);
    return cb(err, res);
  });
}

function starings_for_user (starings, user, cb) {
  ...
  recursive_request(url, starings, {user: user}, fields, reqOptions, cb);
}

function repos_for_orgs (repos, org, cb) {
  const url = `https://api.github.com/orgs/${org}/repos`;
  recursive_request(url, repos, {org: org}, ['name', 'fork'], cb);
}
